  ...
  return (
    <div className="py-2 md:pb-4">
          ...
          >
            {isFutureSale ? (
              <div className="text-center mb-2">
                ...
                <p className="mt-4 text-[0.8rem] leading-relaxed" style={{ color: 'var(--tt-muted-2)' }}>
      ...
      <div className="tt-container tt-container-padding mt-10 !pt-4 border-t" style={{ borderColor: 'var(--tt-border)', opacity: 1 }}>
        ...
        <RelatedProducts categoryId={product.product_categories?.id} tagIds={tag_ids} catIds={cat_ids} locIds={loc_ids} currentSlug={product.slug} excludeId={id} authorId={product.user_id} storeName={tt_location?.name} />
      </div>
...
function RelatedProducts({ categoryId, currentSlug, authorId, tagIds, catIds, locIds, storeName, excludeId }) {
  const { data: products, isLoading } = useProducts({ categoryId, limit: 8 });
...
/* 
  ...
  return (
    <div className="tt-grid-products mb-5">
      ...
      <ProductsView source="custom" excludeId={excludeId}/>
    </div>
  );
}
