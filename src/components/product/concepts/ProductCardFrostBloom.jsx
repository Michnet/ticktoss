...
function GlassUnit({ value, label, show = true }) {
  ...
  return (
    <div className="flex flex-col items-center gap-0.5">
      <div className="flex h-8 w-8 items-center justify-center rounded-lg border border-white/25 bg-white/10 font-['Syne',sans-serif] text-[0.8rem] font-bold text-white shadow-inner backdrop-blur-md">
        {String(value).padStart(2, '0')}
      </div>
      <span className="text-[0.45rem] font-semibold uppercase tracking-wide text-white/55">{label}</span>
    </div>
  );
}
 ...
 */
export default function ProductCardFrostBloom({ product, index = 0 }) {
  ...
  return (
    <motion.div
      ...
      className="group relative flex h-full flex-col overflow-hidden rounded-3xl border border-white/10 shadow-sm transition-transform duration-300 hover:-translate-y-1 hover:shadow-2xl"
    >
      {/* Ambient backdrop — the blurhash itself, not a placeholder.
          backgroundColor (from the average color) paints instantly, even
          during SSR; the fully-decoded blur image swaps in once the
          client-only canvas effect resolves. */}
          <div
      ...
      <Link href={`/products/${slug}`} className="block space-y-3 p-3.5">
        

        {/* Spotlight thumbnail */}
        <div className="relative mx-auto mt-3 aspect-square">
          <img
          ...
          />
          {pct > 0 && !isOutOfStock && (
            <div className="absolute -right-2 -top-2 flex h-10 w-10 items-center justify-center rounded-full border border-white/30 bg-black/40 text-[0.75rem] font-black text-white backdrop-blur-md">
              {Math.round(pct)}%
            </div>
          )}
          {isOutOfStock && (
        ...
        <div className="relative mt-auto overflow-hidden rounded-2xl border border-white/20 bg-white/10 p-3 shadow-lg backdrop-blur-xl">
          ...
          <div className="mt-1 flex items-baseline gap-1.5">
            <span className="font-['Syne',sans-serif] text-[1rem] font-bold text-white">{formatUGX(sale_price)}</span>
            {price > sale_price && (
              <span className="text-[0.68rem] text-white/50 line-through">{formatUGX(price)}</span>
            )}
          </div>
          ...
          <div className="mt-2 [&_button]:!text-white/80 [&_button:hover]:!text-white">
            <ProductActions leftExtraClass='' exClass="-mx-1" product={product} />
          </div>
        </div>
        {/* Top row: category + watchers, dark glass chips for guaranteed contrast */}
        <div className="flex items-center justify-between">
          {category?.name ? (
            <span className="rounded-full border border-white/15 bg-black/30 px-2.5 py-1 text-[0.6rem] font-semibold uppercase tracking-wider text-white/85 backdrop-blur-md">
              {category.name}
            </span>
          ) : <span />}
          {watchers > 0 && (
            <span className="flex items-center gap-1 rounded-full border border-white/15 bg-black/30 px-2 py-1 text-[0.6rem] font-semibold text-white/85 backdrop-blur-md">
